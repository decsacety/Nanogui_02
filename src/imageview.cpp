...
namespace nanogui {
    ...
    void ImageView::setImageCoordinateAt(const Vector2f& position, const Vector2f& imageCoordinate) {
        //Calculate where the new offset must be order to satisfy the image position eqation
    ...
    void ImageView::draw(NVGcontext* ctx) {
        Widget::draw(ctx);
        nvgEndFrame(ctx);//Flush the NanoVG draw stack, not necessary to call nvgBeginFrame

        drawImageBorder(ctx);

        //Calculate several variables that need to be sent to OpenGL in order for the image to be
    ...
    void ImageView::updateImageParamters() {
    ...
    void ImageView::drawWidgetBorder(NVGcontext* ctx) const {
        ...
        nvgRoundedRect(ctx, mPos.x() + .5f, mPos.y() + .5f, mSize.x() - 1,
            mSize.y() - 1, 0);
    ...
    void ImageView::drawImageBorder(NVGcontext* ctx)const {
        ...
        nvgRect(ctx, borderPosition.x() - .5f, borderPosition.y() - .5f,
            borderSize.x() + 1, borderSize.y() + 1);
        nvgStrokeColor(ctx, Color(1.f, 1.f, 1.f, 1.f));
        nvgStroke(ctx);
        nvgResetScissor(ctx);
        nvgRestore(ctx);
    }
    ...
    void ImageView::drawHelpers(NVGcontext* ctx) const {
        // We need to apply mPos after the transformation to account for the position of the 
    ...
    void ImageView::drawPixelGrid(NVGcontext* ctx, const Vector2f& upperLeftCorner,
        ...
        nvgStrokeColor(ctx, Color(1.f, 1.f, 1.f, .2f));
        nvgStroke(ctx);
    }

    void ImageView::drawPixelInfo(NVGcontext* ctx, float stride) const {
        // Extract the image coordinate at the two cornors of the widget/
        ...
        // Extract the position for where to draw the text
    ...
    void ImageView::writePixelInfo(NVGcontext* ctx, const Vector2f& cellPosition,
        ...
        //If no data is provided fot this pixel then simplt return 
