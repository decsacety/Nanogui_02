/*
* src/ colorwheel.cpp--fancy analop widget to select a color value
*/
...
namespace nanogui {
	...
	void ColorWheel::draw(NVGcontext* ctx) {
		...
		nvgFillPaint(vg, paint);
		nvgFill(vg);
		paint = nvgLinearGradient(vg, (r + ax) * .5f, (0 + ay) * .5f, bx, by,
	...
	bool ColorWheel::mouseDragEvent(const Vector2i& p, const Vector2i& rel, int button, int modifiers) {
	...
	ColorWheel::Region ColorWheel::adjustPosition(const Vector2i& p, Region consideredRegions) {
		...
		float ax = std::cos(120.f / 180.f * NVG_PI) * r;
		float ay = std::sin(120.f / 180.f * NVG_PI) * r;
		float bx = std::cos(-120.f / 180.f * NVG_PI) * r;
		float by = std::sin(-120.f / 180.f * NVG_PI) * r;
	...
	void ColorWheel::setColor(const Color& rgb) {
		...
		if (max == min) {
			...
			float d = max - min, h;
			/* float s= 1>0.5 ? d/(2-max-min):d/(max+min); */
			if (max == r)
				h = (g - b) / d + (g < b ? 6 : 0);
			else if (max == g)
				h = (b - r) / d + 2;
