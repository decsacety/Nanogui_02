...
namespace nanogui {
    ...
    TabWidget::TabWidget(Widget* parent) :
        Widget(parent),
        mHeader(new TabHeader(nullptr))// create uning nullptr, add children below
    ...
    void TabWidget::addChild(int /*index*/, Widget* /*widget*/) {
        ...
        throw std::runtime_error(
            "TabWIdget: do not add childrne directly to the TabWidget, create tabs"
            "and add children to the tabs, See TabWidget class documentation for expample usage"
        );
    }
    ...
    void TabWidget::addTab(int index, const std::string& label, Widget* tab) {
        ...
        //of the header will automactically fire when a new tab is added
    ...
    void TabWidget::draw(NVGcontext* ctx) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            nvgBeginPath(ctx);
            nvgRoundedRect(ctx, mPos.x() + .5f, mPos.y() + tabHeight + .5f, mSize.x() - 1,
                mSize.y() - tabHeight - 2, mTheme->mButtonCornerRadius);
