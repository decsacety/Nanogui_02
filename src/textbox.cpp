...
namespace nanogui {
	...
	void TextBox::draw(NVGcontext* ctx) {
		...
		if (mLabel != "") {
			mPos.x() = mLabel.size() * 7 + 15;
			mSize.x() = mPos.x() + mValueTemp.size() * 7;
		}
		

		NVGpaint bg = nvgBoxGradient(ctx,
			mPos.x() + 1, mPos.y() + 1 + 1.0f, mSize.x() - 2, mSize.y() - 2,
			3, 4, Color(0.26f, 0.59f, 0.98f, 0.35f), Color(0.26f, 0.59f, 0.98f, 0.10f));
		NVGpaint fgl = nvgBoxGradient(ctx,
			mPos.x() + 1, mPos.y() + 1 + 1.0f, mSize.x() - 2, mSize.y() - 2,
			3, 4, Color(0.26f, 0.59f, 0.98f, 0.35f), Color(0.26f, 0.59f, 0.98f, 0.10f));
		...
		//背景框
		nvgBeginPath(ctx);
		nvgRoundedRect(ctx, mPos.x() + 1, mPos.y() + 1 + 0.8f, mSize.x() - 2,
			mSize.y() - 5, 0);//原为3
		
		if (mEditable && focused())
			mValidFormat ? nvgFillPaint(ctx, fgl) : nvgFillPaint(ctx, fg2);
		else if (mSpinnable && mMouseDownPos.x() != -1)
			nvgFillPaint(ctx, fgl);
		else
			nvgFillPaint(ctx, bg);
		...
		if (mCommitted) {
			...
			if (mCursorPos > -1) {
				if (mSelectionPos > -1) {
					...
					//draw selection
					nvgBeginPath(ctx);
					nvgFillColor(ctx, nvgRGBA(66, 155, 250, 89));
	...
	bool TextBox::mouseMotionEvent(const Vector2i& p, const Vector2i&/*rel*/
		, int /*butotn*/, int /*modifiers*/) {
		mMousePos = p;
		if (mEditable)
			setCursor(Cursor::Arrow);
		else if (mSpinnable && !focused() && spinArea(mMousePos) != SpinArea::None)
			/*scrolling the arrows*/
			setCursor(Cursor::Hand);
		else setCursor(Cursor::IBeam);
	...
	void TextBox::updateCursor(NVGcontext*, float lastx, const NVGglyphPosition* glyphs, int size) {
	...
	TextBox::SpinArea TextBox::spinArea(const Vector2i& pos) {
		if (0 <= pos.x() - mPos.x() && pos.x() - mPos.x() < 14.f) {/*on scrolling arrows*/
			if (mSize.y()>=pos.y()-mPos.y() && pos.y() - mPos.y() <= mSize.y() / 2.f)
				return SpinArea::Top;/*top part*/
			else if (0.f <= pos.y() - mPos.y() && pos.y() - mPos.y() > mSize.y() / 2.f)
				return SpinArea::Bottom;/*bottom part*/
		}
