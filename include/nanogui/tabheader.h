    ...
    */
    ...
    public:
        ...
        */
        ...
        /// Insets a tab into the tabs collection at the specified index
        void addTab(int index, const std::string& label);

        /*
        * Remove the tab with the specified label and return the index of the label
        * Retrun -1 if there was no such tab
        */
        ...
        /// Retrieve the label of the tab at a specific index
        const std::string& tabLabelAt(int index)const;

        /*
        * Retrieves the index of a specific tab label
        * Retruns the number of tabs (tabsCount) if there is no such tab
        */
        int tabIndex(const std::string& label);

        /*
        * Recalculate the visible range of tabs so that the tab with the specified 
        * index is visible. The tab with the specified index will wither be the 
        * first or last visible onr dfepending on the position relative to the 
        * old visible range
        */
    ...
    private:
        ...
        */
        ...
        public:
            ...
            void drawActiveBorderAt(NVGcontext* ctx, const Vector2i& position, float offset
                , const Color& color);
            void drawInactiveBorderAt(NVGcontext* ctx, const Vector2i& position, float offset,
                const Color& color);
        ...
        private:
            ...
            /*
            * \sturct StringView tabheader.h nanogui/tabheader.h
            * 
