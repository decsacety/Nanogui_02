...
* 
* The text box widget was contributed by me( ei hei ~)
*/
...
namespace nanogui {
	...
	* 
	* brief Fancy taxt box with builtin regular expression-based validation
	* 
	* remark
	...
	*/
	...
	public:
		//How to align the text int the text box 
		...
		void setEditable(bool eidtable);
		...
		///Return the underlying expression specifying valid formats
		const std::string& format() const { return mFormat; }
		///Specify a regular expression specifying valid formats
		void setFormat(const std::string& format) { mFormat = format; }

		///Return the placeeholder text to be displayed while the text box is empty
		const std::string& placeholder() const { return mPlaceholder; }
		///Specify a plaveholder text to be displayed while the text box is empty
		void setPlaceholder(const std::string& placeholder) { mPlaceholder = placeholder; }

		/// Set the /ref Theme used to draw this widget 
		virtual void setTheme(Theme* theme) override;

		// THe callbakc to excute when the value of this Textbox has changed
		std::function<bool(const std::string& str)>  callback() const { return mCallback; }

		//Sets the callback to execute when the value of this TexBox has changed
	...
	public:
		IntBox(Widget* parent, Scalar value = (Scalar)0) :TextBox(parent) {
			...
			setValueIncrement(1);
			setMinMaxValue(std::numeric_limits<Scalar>::lowest(), std::numeric_limits<Scalar>::max());
			setValue(value);
			setSpinnable(false);
		}
		...
		void setMinMaxValue(Scalar minValue, Scalar maxValue) {
	...
	* 
	* brief A specialization of TextBox representing float point values;
	* 
	* Template parameters should be float types e.g.  float double float64_t, etx
	*/
	...
	public:
		...
		void setMaxValue(Scalar max) {
			mMaxValue = max;
		}

		void setMinMaxValues(Scalar min, Scalar max) {
			setMinValue(min); 
			setMaxValue(max);
		}
		...
		virtual bool mouseDragEvent(const Vector2i& p, const Vector2i& rel, int button, int modifiers) override {
			...
			if (mSpinnable && !focused() && button == 2
				/*1<<GLFW_MOUSE_BUTTON_2*/ && mMouseDownPos.x() != -1) {
				int valueDelta = static_cast<int>((p.x() - mMouseDownPos.x()) / float(10));
				setValue(mMouseDownValue + valueDelta * mValueIncrement);
				if (mCallback)
					mCallback(mValue);
